...
- Рабочий вход `/login` (JWT, хеш пароля через argon2); если seed ещё не запускался, первый вход `admin@outerhaven.local / Admin123!` автоматически создаёт админа Михаила.
...
- CORS настроен для `localhost:3000`, `127.0.0.1:3000` и может быть расширен через `WEB_URL`.
