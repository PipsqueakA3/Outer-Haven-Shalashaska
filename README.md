...
## Если PR не мержится из-за конфликтов
Когда одна часть изменений уже влита, проще всего обновить ветку и открыть свежий PR:
```bash
git checkout work
git fetch origin
git rebase origin/main
# решить конфликты в перечисленных файлах
# затем:
git add .
git rebase --continue
```
Если конфликтов слишком много, можно пересобрать ветку заново:
```bash
git checkout -b work-v2 origin/main
git cherry-pick d4d52d3
```
